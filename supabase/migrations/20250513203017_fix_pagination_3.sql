CREATE OR REPLACE FUNCTION public.get_nearby_postings_v4(
  ...
  ) RETURNS TABLE (rows jsonb [], count bigint) LANGUAGE plpgsql
...
END;
$$;
CREATE OR REPLACE FUNCTION public.get_recent_postings_v4(
...
$$;
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.get_nearby_postings_v4(
    double precision,
    double precision,
    bigint,
    bigint,
  ...
  authenticated,
  service_role;
GRANT EXECUTE ON FUNCTION public.get_recent_postings_v4(
    double precision,
    double precision,
    bigint,
    integer,
